...
module.exports = class extends Base {
  ...
  async addUpdateCompanyAction() {
    try {
      const {id, create_time, type, company_id, company_name, usernum, username, telephone, email} = this.post();
      if (this.user.role == 1) {
        if (type == 'add') {
          const companyExistId = await this.model('company').where({
            company_id
          }).select();
          if (!think.isEmpty(companyExistId)) {
            return this.fail('公司ID已经存在');
          }
          const companyExistName = await this.model('company').where({
            company_name
          }).select();
          if (!think.isEmpty(companyExistName)) {
            return this.fail('公司名称已经存在');
          }
          const companyExistLeaderId = await this.model('user').where({
            usernum
          }).select();
          ...
          const create_time = dateTime.getFullYear() + '-' + Number(dateTime.getMonth() + 1) + '-' + dateTime.getDate() + ' ' + dateTime.getHours() + ':' + dateTime.getMinutes() + ':' + dateTime.getSeconds();
          await this.model('company').add({
            company_id, company_name, usernum, username, telephone, email, create_time
          });
          const salt = 'weekly';
          const password = think.md5(salt + '123456');
          ...
          const dateTime = new Date();
          const update_time = dateTime.getFullYear() + '-' + Number(dateTime.getMonth() + 1) + '-' + dateTime.getDate() + ' ' + dateTime.getHours() + ':' + dateTime.getMinutes() + ':' + dateTime.getSeconds();
  ...
  async deleteCompanyAction() {
    const {id, company_id, company_name} = this.post();
    try {
      if (this.user.role == 1) {
  ...
  async addDepartmentAction() {
    console.log("新增部门");
    const {department_id, department_name, type, id, company_id, company_name} = this.post();
    try {
      if (type == 'add') {
        if (this.user.role == 1 && company_id && company_name) {
          const userExistId = await this.model('department').where({
            company_id, department_id
          }).select();
          if (!think.isEmpty(userExistId)) {
            return this.fail('部门ID已经存在');
          }
          const userExistName = await this.model('department').where({
  ...
  async deleteDepartmentAction() {
    const company_id = this.user.company_id || this.post('company_id');
    const {department_id} = this.post();
    try {
  ...
  async getAllMemberListAction() {
    ...
    const company_id = this.post('company_id');
    if (!page) { page = '1' }
